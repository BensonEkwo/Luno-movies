...
const Navbar = () => {
  ...
  return (
    <div className="flex top-0">
      <nav className="top-0 mt-0 pt-3 z-[100] pb-2 fixed flex my-4 w-full justify-between mx-auto items-center flex-wrap bg-blur backdrop-blur-md bg-opacity-50 rounded-b-sm">
        <div className=" flex items-center mx-2 md:mx-4">
          ...
          <div className="ml-4 md:ml-8">
            <form  onFocus={focusin}  className={`group relative rounded-xl py-1 md:py-2 px-2 md:min-w-[50vw] max-w-[50vw] bg-slate-700  items-start flex justify-between leading-tight ${focus&&'border-2 border-slate-200 scale-105 transition-all ease-in-out'}`}>
              <input
                className="focus:outline-none bg-transparent min-w-full text-xs font-normal"
                type="text"
                value={value}
                placeholder="search"
              ...
              />
               ...
               {isSearchOpen? <button onClick={handleSearch} className="absolute right-2 bg-slate-600 rounded-full">
                <IoMdClose/></button>:<div onClick={focusin} className="absolute right-2">
              <IoIosSearch />
              </div>
                 } 
        ...
        <div className="md:hidden flex mx-auto mr-4">
          <button onClick={()=>SetIsOpen(!isOpen)} className="text-2xl transition-all ease-out">
     ...
     {isOpen&&
      ...
      <Link  to='/' className="text-white text-xl flex hover:text-blue-200">
      <IoHome />
        <span className="text-sm ml-1">  Home</span></Link>
      <Link  to='/Categories' className="text-white text-xl flex hover:text-blue-200">
      <BiSolidCategory />
        <span className="text-sm ml-1">Categories</span></Link>
      <Link  to='/Watchlist' className="text-white text-xl flex hover:text-blue-200">
      <HiMiniQueueList />
        <span className="text-sm ml-1">Watchlist</span></Link>
      <Link  to='/Tvshows' className="text-white text-xl flex hover:text-blue-200">
      <PiTelevisionSimple />
        <span className="text-sm ml-1">Tv Shows</span></Link>
      <Link  to='/' className="text-white text-xl flex hover:text-blue-200">
      <RiMovie2Fill />
        <span className="text-sm ml-1">Movies</span></Link>
      </div>
      </div>
     }
